...
* 若表T没有字段k，那么执行`select * from T where k=1`时会在什么阶段查到异常
...
Mysql会给每个sql线程分配一块内存用于排序，称为 sort_buffer，从主键索引获取到完整数据记录后存放到 sort_buffer 中，根据 order by 字段进行快速排序

如果待排序的数据量小于 sort_buffer_size，则排序过程在内存中完成，而过待排序数据量大于 sort_buffer_size 则利用磁盘文件来辅助排序（外排序）

#### rowid 排序

全字段排序会将所有需要排序的记录行通盘取出，放到sort_buffer中进行排序操作，而过记录字段数较多时，sort_buffer存放不下待排序记录时，会导致性能下降

即：**待排序的记录行长度过大时，排序性能会受到影响**，因此需要转化排序方法，Mysql中通过`SET max_length_for_sort_data = 16;` 来设定字段长度阈值，超出这个阈值后，会使用其他排序方法。

为了解决这个问题，提出了rowid排序方法，即每次仅将待排序字段与主键存放到sort_buffer中，排序完成后再通过主键回表查询，即以时间换空间，增加了回表查询补全记录数据的时间，但是会大大降低对sort_buffer空间的占用

#### 索引排序

当待排序字段有可用索引时，由于索引本身就是有序的，则不需要再次进行排序过程，直接按照在索引中的序获取数据并透出
...
通过主键索引与非主键索引进行查询的区别在于： 主键索引查到对应的叶子节点之后可以直接拿到记录的内容，而非主键索引查到对应的叶子节点之后拿到的是主键的值，还需要再
借助主键索引进行记录的查询，这个再次查询的过程称为”回表“，因此非主键索引会比主键索引多一次查询过程
...
![](https://pic3.zhimg.com/80/v2-10898f59099e5cc27402ac24d095b08a_hd.jpg)
...
**两阶段锁**：在InnoDB事务中，行锁是在需要的时候加上，也就是需要对记录进行变更操作时候对所在记录行加锁，但是**解锁却不是在用完就进行，而是在事务结束时才会释放**

由于存在上述设定，所以当事务中存在多条记录需要加锁时，建议将最有可能造成锁冲突的加锁操作（对指定记录行变更）放到最后，以减少其他事务的等待时间

**行锁是加在索引上的**

以`select for update`为例，其会为记录加上排他锁（X锁）

1. 如果where查询条件是主键索引（聚簇索引），则会对主键索引记录加上X锁
2. 如果where查询条件是普通索引（辅助索引），则会对辅助索引记录加上X锁，并且对回表到的主键索引记录加上X锁，如果该普通索引整好是唯一索引则仅会查询到一条记录，如果不是唯一索引，则会查询到多条记录，对多条索引记录及其对应的主键索引记录加上X锁
3. 对于非唯一索引，但是由于没有唯一性约束，这时候其他事务仍然可以给插入相同条件的数据，导致插入前后锁看到的查询结果存在差别，这就是**幻读**，这里InnoDB引入了间隙锁，也就是会锁住相同条件的数据插入间隙，避免幻读产生
4. 对于无索引字段，会锁住全表（给所有记录加上行锁），然后再做匹配判断，RC级别下，对于不匹配的行解除行锁，匹配到的行保留锁直到事务提交，RR级别下，事务结束前不会对不符合记录做行锁解除

![](https://pic3.zhimg.com/80/v2-a4285faa5dec95aa2d0078a23962b48a_hd.jpg)

**Next Key Lock** : 记录锁 + 间隙锁，既会锁定指定记录，也会锁定相同条件下的插入间隙（防止幻读）

**插入意向锁**：实际上是间隙锁，会在进行insert操作时候使用，其作用包括：

* 和 next-key 锁互斥，会阻塞next-key锁，防止数据插入，避免幻读
* 插入意向锁之间是兼容的，允许相同间隙不同数据的并发插入

间隙锁之间是兼容的，插入意向锁之间也是兼容的，间隙锁与插入意向锁之间是冲突的

**只有在RR隔离级别下才存在间隙锁**

#### Next-Key Lock

在RR隔离级别下，普通的查询读取是快照读方式，即不能看到其他事务提交的变更，因此，“幻读”仅仅只在读取操作是“当前读”中存在

由于其他事务的操作，导致当前事务在进行两次“当前读”时候得到的数据不一致，就是幻读所造成的影响，解决幻读的措施可以通过加锁的方式，但是如果只对当前存在的记录加锁，没法解决因为新插入数据导致的幻读问题，所以解决幻读还必须要引入间隙锁(Gap Lock)

不仅可以通过行锁锁住索引数据行，同样可以通过间隙锁锁住数据行之间的间隙，但是间隙锁跟行锁之间是不一样的
...
行锁跟行锁之间是存在冲突关系的，而间隙锁之间是不直接存在冲突的，间隙锁只与“往该间隙插入一条记录”操作存在冲突

**行锁与间隙锁合称 next-key lock**，每个next-key lock都是前开后闭区间

但是，间隙锁的引入，可能导致同样的语句锁住更大的范围，会影响到并发度
...
* 优化2： 索引上的等值查询，向右遍历且最后一个值不满足等值条件的时候，next-key lock退化为间隙锁
* 1bug： 唯一索引上的范围查询会访问到不满足条件的第一个值为止
